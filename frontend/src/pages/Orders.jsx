...
const Orders = () => {
          ...
          {filterTags.length > 0 && (
            <div className="flex flex-row gap-2 flex-wrap px-4 pb-2">
              {filterTags.map((tag, idx) => (
                <button
                  key={`${tag}-${idx}`}
                  className={`flex flex-row gap-2 items-center p-2 text-xs font-normal rounded hover:line-through cursor-pointer ${
                    isDark ? "bg-gray-800" : "bg-[#e7e7e7]"
                  }`}
  ...
  return (
    <div
      className={`w-full sm:px-5 px-1 sm:py-5 pb-10 lg:min-h-[calc(100dvh-112px)] md:min-h-[calc(100dvh-80px)] min-h-[calc(100dvh-112px)] ${
        isDark
    ...
    >
      ...
      >
        ...
        <div className="lg:hidden">
          <button
            className={`flex flex-row justify-center items-center gap-2 text-lg cursor-pointer ${
        ...
        <div
          className={`absolute top-full z-10 right-0 w-60 sm:w-80 h-fit shadow-2xl lg:hidden ${
            openFilters
        ...
        <div className="h-fit w-full relative">
          ...
          >
            {loading ? (
              ...
              <div
                className={`easeIn lg:h-[75dvh] h-[78dvh] rounded flex justify-center items-center border-2 ${
                  isDark
                    ? "bg-gray-900 text-gray-300 border-gray-800 shadow-[0px_0px_12px_rgba(0,0,0,0.5)]"
              ...
              <>
                {orders.map((order) => (
                  <div
                    key={order.orderId}
                    className={`rounded-xl border-2 min-h-fit overflow-hidden flex flex-col ${
                      isDark
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                            ...
                            {order?.paymentMethod && (
                              <span>Method: {formatName(order.paymentMethod)}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span
                            className={`flex items-center gap-2 text-sm font-semibold ${
                              statusColors[order?.orderStatus] || "text-gray-500"
                            }`}
                          >
                            <IoMdRadioButtonOn />
                    ...
                    <div className="divide-y divide-gray-200/10">
                      {order?.orderItems?.map((item, idx) => (
                        <div
                          ...
                          className={`px-4 sm:px-5 py-4 cursor-pointer transition-colors duration-200 ${
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4 min-w-0">
                              ...
                              <div className="min-w-0 flex flex-col">
                                ...
                                <span className="text-sm text-gray-500 mt-1 sm:hidden flex items-center gap-1">
                                  <IoMdRadioButtonOn
                                    className={
                            ...
                            <div className="hidden sm:flex flex-col items-end gap-2 shrink-0">
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigate(
                                    `/my-account/my-orders/${idx}/${order.orderId}`
                                  );
                                }}
                              >
                                View Item
                ...
                <div className="w-full flex justify-center items-center font-semibold">
                  {orders.length < totalOrders ? (
                    <button
                      className={`text-[#FF6F61] px-4 py-2 rounded transition-shadow duration-200 hover:shadow-[0px_0px_8px_rgba(0,0,0,0.15)] border-2 cursor-pointer w-fit ${
                        isDark
                    ...
                    <span
                      className={`text-[#FF6F61] px-4 py-2 rounded border-2 ${
                        isDark
                          ? "bg-gray-900 border-gray-800"
                          : "bg-white border-[#87878730]"
                      }`}
          ...
          <div
            className={`pointer-events-none absolute z-10 bottom-0 left-1/2 -translate-x-1/2 transition-all duration-300 ${
